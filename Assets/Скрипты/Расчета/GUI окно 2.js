...
function OnGUI ( ) {
    if (b_display)//Показывает все
    {
        ...
        // Окно с текстом
        GUI.Box (Rect (5,5,740,250), "Согласно Строевому уставу:"); 
        ...
        GUI.Label (Rect (20,180,700,200) , "    Для посадки экипажей в машины подаётся команда (сигнал): 'ПО МЕСТАМ'."); 
        GUI.Label (Rect (390,220,700,200) , "Расчет построен: "); 
        ...
        //Кнопка "Далее"
        if (GUI.Button (Rect (530,215,180,30), "ПО МЕСТАМ")) { 
            ...
            var audioBEG : AudioSource = GetComponent.<AudioSource>(); //Для аудио привода тумблера
            audioBEG.Play();// аудио тумблера ПРИВОД
            Animate();
    ...
    function Animate() {
        ...
        var ZAVODIscript = GameObject.Find("РАСЧЕТ С АНИМАЦИЕЙ");
        ZAVODIscript.GetComponent("ЗАВОДИ").enabled = true;
